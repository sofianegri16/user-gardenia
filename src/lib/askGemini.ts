...
export interface AskGeminiPayload {
  user_role: string;
  team_state: {
    energy_avg: number;
    pressure_avg: number;
...
/**
 * Calls the ask-gemini Edge Function to get AI assistant responses
 * @param payload The data containing user role, team state, and question
 * @returns The AI response with the answer
 */
export const askGemini = async (payload: AskGeminiPayload): Promise<AskGeminiResponse> => {
  try {
    ...
    if (!data) {
      console.error('No data received from ask-gemini function');
      throw {
    ...
    if (data.error) {
      console.error('Error returned from ask-gemini function:', data);
      ...
      if (data.errorType === 'gemini_api_error') {
        throw {
    ...
    if (!data.answer) {
      console.error('Invalid response from ask-gemini function:', data);
      throw {
    ...
    console.error('Error in askGemini function:', error);
    
    // If the error is already formatted as AskGeminiError, rethrow it
    if (error.type) {
      throw error;
    }
 ...
 */
export const prepareTeamStateForGemini = (teamData: TeamEmotionalData): AskGeminiPayload['team_state'] => {
